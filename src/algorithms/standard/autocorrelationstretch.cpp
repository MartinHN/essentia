...
#include "autocorrelationstretch.h"
...
const char* AutoCorrelationStretch::name = "AutoCorrelationStretch";
const char* AutoCorrelationStretch::description = DOC(
...
void AutoCorrelationStretch::configure() {
  _pow =  parameter("stretch").toReal();
_nS = parameter("timeM").toReal();
...
void AutoCorrelationStretch::compute() {
  ...
  float numS = _nS*int(_fftBuffer.size());
  //Stretching
  for (int i=0; i<int(_fftBuffer.size()); i++) {
  float c = pow((Real)i/_fftBuffer.size(),(Real)_pow)*numS/c;
  	if(c>int(_fftBuffer.size()))break;
  	else{
  	int x0 = (int)c;
  	float resi = x0-c;
  _fftBuffer[i] = complex<Real>(((1-resi)*_fftBuffer[c].real()+(resi)*_fftBuffer[c+1].real())/2.0,0.0);
  }
  }
